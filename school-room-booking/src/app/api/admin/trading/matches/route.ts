...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabaseAdmin
      ...
      .select(`
        ...
        listing_a:trade_listings!trade_matches_listing_a_id_fkey(
          ...
          courses:trade_listing_courses(id, course_name, course_code, type)
        ),
    ...
    const result = (matches || []).map(m => {
      const la = m.listing_a as unknown as { id: string; status: string; user: { id: string; full_name: string; username: string } | null; courses: { id: string; course_name: string; course_code: string; type: string }[] } | null
      const lb = m.listing_b as unknown as { id: string; status: string; user: { id: string; full_name: string; username: string } | null; courses: { id: string; course_name: string; course_code: string; type: string }[] } | null

      const mapCourses = (courses: { id: string; course_name: string; course_code: string; type: string }[]) =>
        courses.map(c => ({ id: c.id, courseName: c.course_name, courseCode: c.course_code, type: c.type }))
